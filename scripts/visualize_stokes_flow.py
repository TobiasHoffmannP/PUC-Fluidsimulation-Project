...
M, C = build_mass_and_convection(nodes_coords, triangles, u)
apply_periodic_bc(M, pairs)
apply_periodic_bc(C, pairs)
A_c  = M + DT * D * A_stiffness
apply_periodic_bc(A_c, pairs) 
...
#
    ...
    C_u  = build_mass_and_convection(nodes_coords, triangles, u)[1]


    div_u_nodes = calculate_divergence(nodes_coords, triangles, u)

    G_diag = DT * (M_lumped_diag * div_u_nodes)      # shape (N,)

    # we must give slave nodes the same value as their master to keep
    # periodic symmetry
    for master, slave in pairs:
        G_diag[slave] = G_diag[master]

    # assemble the step matrix
    A_c_step = M + DT * (C_u + D * A_stiffness) + np.diag(G_diag)
    apply_periodic_bc(A_c_step, pairs)               # keep symmetry

    rhs_c = M @ c                                    # no change here
    ...
    if step > 0:
        ...
        print(f"c min and c max: {c.min()}, {c.max()}")
